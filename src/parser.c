...
static void __parse_files_list(Bencode *files_list, TorrentMeta *meta)
{
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!length_b || length_b->type != BE_INTEGER) {
      fprintf(stderr,
              "Warning: file entry %zu missing/invalid length — skipping\n", i);
      continue;
    }
    ...
    if (!path_b || path_b->type != BE_LIST || path_b->len == 0) {
      fprintf(stderr,
              "Warning: file entry %zu missing/invalid path — skipping\n", i);
      continue;
    }
    ...
    for (size_t j = 0; j < path_len; ++j) {
      ...
      if (!pc || pc->type != BE_STRING) {
        fprintf(stderr,
                "Warning: file entry %zu has non-string path component at %zu "
                "— skipping\n",
                i, j);
        bad = 1;
        break;
