...
void __print_parsed_data(Bencode *data)
{
  ...
  case BE_LIST:
    ...
    for (size_t i = 0; i < data->len; i++) {
      ...
      i < data->len - 1 ? printf(", ")
                        : printf("]"); // dont print "," at the end of the list
    }
    break;
  case BE_DICT:
    printf("{");
...
void free_be(Bencode *b)
{
  if (b->type == BE_STRING) {
    ...
    for (size_t i = 0; i < b->len; i++) {
      free_be(b->value.list[i]); // value.list and value.dict reffers to the
                                 // same memory space as it is a union
    }
    free(b->value.list);
  }
  free(b);
...
// {
//     // const char *bencode_data =
//     "di-42e4:spami21e5:Helloli69ei420eed2:Hi3:supee"; const char
//     *bencode_data = read_file("./torrents/toa-btd.torrent");
