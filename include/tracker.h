...
typedef struct
{
    uint32_t interval; // seconds until next re-announce (shit goes wrong must RESPECT!!!)
 ...
 *
 * Parameters:
 *   url        - tracker announce URL, e.g. "udp://tracker.opentrackr.org:1337/announce"
 *   info_hash  - 20-byte raw SHA1 from get_info_hash()
 *   peer_id    - 20-byte peer ID from generate_peer_id()
